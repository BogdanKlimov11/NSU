...
#include <QElapsedTimer> // Для измерения времени между кадрами (для расчета FPS)
...
signals:
    // Сигнал для передачи текущей частоты кадров (FPS)
...
private:
    ...
    // Параметры объекта
    QVector3D cube_color{1.0f, 0.0f, 0.0f}; // Цвет куба (RGB, начальный: красный)
    ...
    QVector3D rotationAxis{0., 0., 1.}; // Ось вращения объекта (по умолчанию: ось Z)
    QBasicTimer timer; // Таймер для периодического обновления сцены

    // Переменные для расчета FPS
    QElapsedTimer fpsTimer; // Таймер для измерения времени между кадрами
    int frameCount = 0; // Счетчик кадров для расчета FPS
    int currentFps = 0; // Текущая частота кадров (FPS)
};
