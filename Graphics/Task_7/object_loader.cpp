...
bool Object::loadObject(const QString& filePath, float scale) {
    ...
    while (!in.atEnd()) {
        ...
        if (tokens[0] == "mtllib" && tokens.size() >= 2) {
            ...
            auto it = std::find_if(materials.begin(), materials.end(), [&materialName](const Material& mat) {
            ...
            for (int i = 1; i <= 3; ++i) {
                ...
                QVector3D normal = (normalIndex >= 0 && normalIndex < (int)tempNormals.size())
                                       ? tempNormals[normalIndex] : QVector3D(0, 0, 1);
                QVector2D texture = (textureIndex >= 0 && textureIndex < (int)tempTextures.size())
                                        ? tempTextures[textureIndex] : QVector2D(0, 0);
