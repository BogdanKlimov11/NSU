...
void test_empty_needle() {
    const char* result = boyer_moore_search("abc", "");
...
void test_needle_longer() {
    const char* result = boyer_moore_search("short", "very long needle");
    assert(result == NULL);
    printf("Test needle longer passed\n");
}

void test_exact_match() {
    const char* result = boyer_moore_search("hello world", "hello world");
    assert(result != NULL && strcmp(result, "hello world") == 0);
    printf("Test exact match passed\n");
}

void test_substring_at_start() {
    const char* result = boyer_moore_search("hello world", "hello");
    assert(result != NULL && strcmp(result, "hello world") == 0);
...
void test_large_text() {
    ...
    const char* result = boyer_moore_search(haystack, "needle");
