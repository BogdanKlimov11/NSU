...
void test_substring_at_end() {
    const char* result = my_strstr("hello world", "world");
...
void test_repeated_pattern() {
    const char* result = my_strstr("ABC ABCDAB ABCDABCDABDE", "ABCDABD");
    assert(result != NULL && strcmp(result, "ABCDABDE") == 0);
    printf("Test repeated pattern passed\n");
}
...
int main() {
    ...
    printf("All KMP tests passed successfully!\n");
    return 0;
}
